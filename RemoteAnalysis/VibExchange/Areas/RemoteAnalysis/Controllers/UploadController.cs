...
namespace VibExchange.Areas.RemoteAnalysis.Controllers
{
    ...
    public class UploadController : Controller
    {
        ...
        {
            string testPath = ConfigurationManager.AppSettings["newpath"].ToString();  
                ...
                else if (file.ContentLength > 0)
                {
                    ...
                    else
                    {
                        using (DBClass context = new DBClass())
                        {
                         ...
                         // var path = Path.Combine(testPath, fname);
        ...
        //[HttpPost]
        //public ActionResult DataUpload1(FormCollection formCollection, HttpPostedFileBase[] files, string ReturnUrl)
        //{
                            ...
                            using (DBClass context = new DBClass())
                            {
                                ...
                                if (dtPoint.Rows.Count > 0)
                                {
                                    foreach (DataRow pointitem in dtPoint.Rows)
                                    {
                                        ...
                                        point.lstUnit = getUnitDetail(point.PointID, Session["UserName"].ToString());
        ...
        public ActionResult DisplayGraph(string nodeid)
        {
            ...
            ViewBag.amplitudeUnit = ChartData.amplitudeUnit; // + (ChartData.overallValue * 1.4142).ToString() + "Overall Peak";
        ...
        public JsonResult GetChartData(string FileID)
        {
            ...
            ArrayList RPMFaultFrequency = new ArrayList();
            RPMFaultFrequency = GetRPMOrderFrequencies(FileID,"4");
            var data = ChartData.GetData(FileID, Session["UserName"].ToString(), BearingFaultFrequency, RPMFaultFrequency);
        ...
        [HttpGet]
        public ActionResult TWFTFFT()
        {
            string PID = "";
            ConversionFWTtoFFT ff = new ConversionFWTtoFFT();
            ff.convId = PID;
            return PartialView("_changeTWFtoFFT", ff);
        }

        [HttpGet]
        public ActionResult ChangeUnit()
        {
            Unit unit = new Unit();
            string id = "";
            // unit.uid = id;
            ...
            else { return PartialView("_changeXUnit", unit); }

        }

        [HttpGet]
        public ActionResult ChangeRPM()
        {          
            //return PartialView("_changeRPM");
            Unit unit = new Unit();
            string id = "";
            // unit.uid = id;
            if (id == "xUnit")
            {
                return PartialView("_changeRPM", unit);
            }

            //else { return PartialView("_changeYUnit", unit); }

            else { return PartialView("_changeRPM", unit); }
        }

        [HttpPost]
        //public ActionResult GetRPMDtl(string order,string pointId)

        public JsonResult GetRPMDtl(string FileID)
        {
            //PointData unit = new PointData();
          
           FileID = Convert.ToString(Session["CurrentNodeID"]);
           // GetRPMValues("sanjayYadav169", "Point348");
            string strDDLValue = Request.Form["RPMUPTO"].ToString();
            Session["strDDLValue"] = strDDLValue;
            ArrayList RPMFaultFrequency = new ArrayList();
           // RPMFaultFrequency = GetRPMOrderFrequencies("Point348", strDDLValue);
            RPMFaultFrequency = GetRPMOrderFrequencies(FileID, strDDLValue);
            ArrayList BearingFaultFrequency = new ArrayList();
            //BearingFaultFrequency = GetBearingFaultFrequencies("Point348");  
            BearingFaultFrequency = GetBearingFaultFrequencies(FileID);
            var data = ChartData.GetData(FileID, Session["UserName"].ToString(), BearingFaultFrequency, RPMFaultFrequency);
            return Json(data, "True", JsonRequestBehavior.AllowGet);
        ...
        public ArrayList GetRPMOrderFrequencies(string spointid, string order)
        {           
            string _Rpm = null;
            ArrayList RPMFaultFrequencies = new ArrayList();
            try
            {
                //string Rpm = "1";
                using (DBClass context = new DBClass())
                {
                    ...
                    {
                        foreach (DataRow dr in dtMachineInfo.Rows)
                        {
                            _Rpm = Convert.ToString(dr["rpmdriven"]);
                        }
                    }
                }
                for (int j = 1; j <=(Convert.ToInt32(order)); j++)
                {
                    RPMFaultFrequencies.Add("RMP = " + Convert.ToString(Convert.ToDouble(Convert.ToDouble(_Rpm.ToString()) / 60)*j));                  
                }
            }
            catch (Exception)
            {
            ...
            return RPMFaultFrequencies;
        }



        public void GetRPMValues(string order ,string Pointid)
        {
            string Pulse = "1"; string Rpm = "1";
            string[] RPMValues = new string[2];
            double[] lstRPM = new double[1];
            try
            {
                using (DBClass context = new DBClass())
                {
                    ...
                    if (dtMachineInfo.Rows.Count > 0)
                    {
                        foreach (DataRow dr in dtMachineInfo.Rows)
                        {
                            Rpm = Convert.ToString(dr["rpmdriven"]);
                        }
                    }
                    ...
                    if (dtUnit.Rows.Count > 0)
                    {
                        foreach (DataRow dr in dtUnit.Rows)
                        {
                            lstUnit.Add(new Unit
                            {
                                ...
                                UnitType = Convert.ToString(dr["ParameterType"]),
                                UnitName = Convert.ToString(dr["Unit"])
                            });
                        }
                    }
                }
        ...
        public ActionResult getUnitDetailX(string nodeid, string userid)
        {
            //ChartData.NodeID = Convert.ToString(TempData["Nodeid"]);
            ViewBag.NodeID = Convert.ToString(nodeid);// Convert.ToString(TempData["Nodeid"]);
            ViewBag.xUnit = ChartData.xAxisUnit;
            ViewBag.yUnit = ChartData.yAxisUnit;
            ViewBag.Overall = ChartData.overallValue;
        ...
        private List<Unit> getUnitDetail(string pointid, string userid)
        {
            List<Unit> lstUnt = new List<Unit>();
            try
            {
                getUnitDetailX(pointid, userid);
                using (DBClass context = new DBClass())
                {
                    context.AddParameter("@PointID", pointid);
                    context.AddParameter("@UserName", userid);
                    DataTable dtUnt = context.getData("getFalutFrequencyListbyPointID", CommandType.StoredProcedure);
